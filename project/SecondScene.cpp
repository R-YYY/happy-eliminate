...
bool Second::init()
{
    ...
    //背景
    Sprite* background1 = Sprite::create("onepiece2.jpg");
    ...
    //目标分数
    auto goal = LabelTTF::create("Goal: 100000", "Arial", 30);
    ...
    //更新
    schedule(schedule_selector(Second::update), 0.5);

    //添加鼠标触摸事件
    auto listener1 = EventListenerTouchOneByOne::create();
    listener1->onTouchBegan = CC_CALLBACK_2(Second::onTouchBegan, this);
    listener1->onTouchMoved = CC_CALLBACK_2(Second::onTouchMoved, this);
    listener1->onTouchEnded = CC_CALLBACK_2(Second::onTouchEnded, this);
...
//产生方块
void Second::createSprite(int r, int c)
{
...
void Second::update(float time)
{
    ...
    if(!isAtion && !isNeedFilled)
        if (gameSteps <= 0 && gameScore < 100000)//没有步数了且未达到目标分数，结束游戏
        {
...
void Second::checkAndRemove()
{
    for (int i = 0; i < MAX_ROW; i++)
    {
        for (int j = 0; j < MAX_COL; j++)
        {
            ...
            else if (colList.size() >= 3 && colList.size() < 5 && rowList.size() >= 3 && rowList.size() < 5)//类似于交叉路口
            {
                for (count = rowList.begin(); count != rowList.end(); count++)
                {
                    markRemove((SpriteType*)*count);
...
//标记需要消除的方块
void Second::markRemove(SpriteType* spr)
{
    spr->setIsNeedRemove(true);
}

//标记消除过程产生特殊方块的方块
void Second::markNextToSpecial(SpriteType* spr)
{
    spr->setIsNextToSpecial(true);
}

//检查行消除
void Second::checkRow(SpriteType* spr, list<SpriteType*>* lineList)
{
...
//检查列消除
void Second::checkCol(SpriteType* spr, list<SpriteType*>* lineList)
{
    lineList->push_back(spr);
...
//特殊方块的消除
void Second::specialRemove(SpriteType* spr)
{
    if (spr == NULL || !spr->getIsSpecial())
        return;
...
//获取触摸位置的方块
SpriteType* Second::getTouchedSprite(Vec2& position)
{
    Rect rect;
...
//触摸中
bool Second::onTouchMoved(Touch* touch, Event* event)
{
    return true;
}

//触摸结束
bool Second::onTouchEnded(Touch* touch, Event* event)
{
    SpriteType* spr;
...
bool Second::swapSpecialSperite()
{
